...
# Set variables for parameters: T_val and B_val, margin-top and margin-bottom, respectively
T_2160=680
B_2160=750
T_1440=500
B_1440=550
...
if ((resolution >= 2160)); then
    T_val=$(awk "BEGIN {printf \"%.0f\", $T_2160 * 2160 * $hypr_scale / $resolution}")
    ...
    T_val=$(awk "BEGIN {printf \"%.0f\", $T_1600 * 1600 * $hypr_scale / $resolution}")
    ...
    T_val=$(awk "BEGIN {printf \"%.0f\", $T_1440 * 1440 * $hypr_scale / $resolution}")
    ...
    T_val=$(awk "BEGIN {printf \"%.0f\", $T_1080 * 1080 * $hypr_scale / $resolution}")
    ...
    T_val=$(awk "BEGIN {printf \"%.0f\", $T_720 * 720 * $hypr_scale / $resolution}")
